...
namespace EmployeeRecord.Services
{
    public class EmpRecordService
    {
        ...
        public EmpRecordService(EmployeeContext employeeContext)
        {
            this._dbContext = employeeContext;
        }

        public Boolean SaveNew(String taskName, String taskDesc, String clientName,
                            String date, String startTime, String endTime, String duration,String projId)
        {
            try
            {
                int pid = Int16.Parse(projId);
                Project proj = new Project();
                proj = _dbContext.project.Where(x => x.projectId == pid).FirstOrDefault();
                Models.EmpTask task = new Models.EmpTask();
                          ...
                          String date, String startTime, String endTime, String duration,String projId)
        {
            try
            {
                ...
                task.Duration = duration;
                task.project = proj;
                _dbContext.EmpTask.Update(task);
                _dbContext.SaveChanges();
        ...
        public List<EmpTask> getEmpTaskByDate(String date)
        {
            List<EmpTask> taskList = new List<EmpTask>();
            //string d = date.Replace("/", "-");
            string d = changeDateFormat(date);

            taskList = _dbContext.EmpTask.Where(x => x.Date == d).ToList();
            return taskList;
        }
